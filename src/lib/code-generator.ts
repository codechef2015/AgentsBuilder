...
import { generateModelCode, generateModelImports } from './model-code-generator';
import { migrateProviderName } from './model-providers';
import { generateAgentConfigImports, generateAgentConfigKwargs, generateInvocationLimits, generateStructuredOutputCode, getStructuredOutputKwarg } from './agent-config-codegen';
import { generateGuardrailsImports, generateBedrockGuardrailKwargs, generateAgentControlSetup, generateAgentControlPluginKwarg, generateCustomGuardrailHookCode, getCustomGuardrailHookKwarg } from './guardrails-codegen';
import { generateObservabilityImports, generateOtelSetupCode, generateAgentSopCode } from './observability-codegen';
import { generateAdvancedImports, generateMemorySetupCode, getMemoryToolKwarg, generateBidirectionalStreamingCode } from './advanced-features-codegen';
import { generatePluginsImports, generateGoalLoopCode, generateHumanInLoopCode, getHumanInLoopHookKwarg, getCommunityToolNames, generateMcpInstrumentationCode } from './plugins-codegen';
...
export function generateStrandsAgentCode(
  ...
  // Check if A2A agent nodes are used
  const hasA2ANodes = nodes.some(node => node.type === 'a2a-agent');
  if (hasA2ANodes) {
    imports.add('from strands.agent.a2a_agent import A2AAgent');
  }

  // Check if workflow nodes are used
  const hasWorkflowNodes = nodes.some(node => node.type === 'workflow');
  if (hasWorkflowNodes) {
    imports.add('from strands_tools import workflow');
  }
  ...
  try {
    ...
    // Add imports for all other model providers used
    const providerIds = allAgentNodes
      .map(node => node.data?.modelProvider || 'bedrock')
      .map((p: string) => migrateProviderName(p));
    const modelImports: string[] = generateModelImports(providerIds);
    modelImports.forEach((imp: string) => {
      imports.add(imp);
    });

    // Add imports for agent configuration (retry, conversation manager, session, etc.)
    allAgentNodes.forEach(node => {
      const configImports = generateAgentConfigImports(node.data || {});
      configImports.forEach(imp => imports.add(imp));
    });

    // Add imports for guardrails (Bedrock guardrails, Agent Control, custom hooks)
    allAgentNodes.forEach(node => {
      const guardrailImports = generateGuardrailsImports(node.data || {});
      guardrailImports.forEach(imp => imports.add(imp));
    });

    // Add imports for observability (OpenTelemetry)
    allAgentNodes.forEach(node => {
      const otelImports = generateObservabilityImports(node.data || {});
      otelImports.forEach(imp => imports.add(imp));
    });

    // Add imports for advanced features (memory, callback handler, etc.)
    allAgentNodes.forEach(node => {
      const advancedImports = generateAdvancedImports(node.data || {});
      advancedImports.forEach(imp => imports.add(imp));
    });

    // Add imports for plugins (goal loop, HITL, community tools, MCP OTEL)
    allAgentNodes.forEach(node => {
      const pluginImports = generatePluginsImports(node.data || {});
      pluginImports.forEach(imp => imports.add(imp));
    });
    
    // Validate mandatory nodes
    if (agentNodes.length === 0 && orchestratorNodes.length === 0 && swarmNodes.length === 0 && a2aNodes.length === 0 && workflowNodes.length === 0) {
      errors.push('No agent nodes found. At least one agent, orchestrator agent, swarm, A2A agent, or workflow is required.');
    }
    ...
    // Generate A2A agent code if A2A nodes exist
    if (a2aNodes.length > 0) {
      a2aNodes.forEach((a2aNode) => {
        const a2aCode = generateA2AAgentCode(a2aNode);
        code += a2aCode + '\n\n';
      });
    }

    // Generate workflow code if workflow nodes exist
    if (workflowNodes.length > 0) {
      workflowNodes.forEach((workflowNode) => {
        const workflowCode = generateWorkflowCode(workflowNode);
        code += workflowCode + '\n\n';
      });
    }

    // Generate main execution code
    const allExecutableAgents = [...agentNodes, ...orchestratorNodes, ...swarmNodes, ...a2aNodes, ...workflowNodes];
...
function generateAgentModelOnly(
  ...
  // Use modelId (dropdown providers) or modelName (text input providers)
...
function generateAgentCode(
  ...
  // Find connected tools + memory tool
  const connectedTools = findConnectedTools(agentNode, allNodes, edges);
  const memoryToolName = getMemoryToolKwarg(data);
  const allAgentToolCodes = [...connectedTools.map(tool => tool.code)];
  if (memoryToolName) {
    allAgentToolCodes.push(memoryToolName);
  }
  const toolsCode = allAgentToolCodes.length > 0
    ? `,\n    tools=[${allAgentToolCodes.join(', ')}]`
    : '';
  ...
  // Generate advanced agent config kwargs (retry, conversation manager, session, structured output)
  const agentConfigKwargs = generateAgentConfigKwargs(data);
  const structuredOutputCode = generateStructuredOutputCode(data);
  const structuredOutputKwarg = getStructuredOutputKwarg(data);

  // Generate guardrails code
  const agentControlSetup = generateAgentControlSetup(data);
  const customGuardrailHookCode = generateCustomGuardrailHookCode(data);
  const agentControlPluginKwarg = generateAgentControlPluginKwarg(data);
  const customGuardrailHookKwarg = getCustomGuardrailHookKwarg(data);
  const bedrockGuardrailKwargs = generateBedrockGuardrailKwargs(data);

  // Build plugins list
  const plugins: string[] = [];
  if (agentControlPluginKwarg) {
    plugins.push(agentControlPluginKwarg);
  }

  // Build hooks list
  const hooks: string[] = [];
  if (customGuardrailHookKwarg) {
    hooks.push(customGuardrailHookKwarg);
  }

  // Build the extra kwargs string
  const extraKwargs = [
    ...agentConfigKwargs,
    ...(structuredOutputKwarg ? [structuredOutputKwarg] : []),
    ...(plugins.length > 0 ? [`plugins=[${plugins.join(', ')}]`] : []),
    ...(hooks.length > 0 ? [`hooks=[${hooks.join(', ')}]`] : []),
  ];
  const extraKwargsStr = extraKwargs.length > 0
    ? ',\n    ' + extraKwargs.join(',\n    ')
    : '';

  // Structured output class definition (placed before Agent instantiation)
  const structuredOutputDef = structuredOutputCode ? `\n${structuredOutputCode}\n` : '';

  // Pre-agent setup code (Agent Control init, custom hook class, OTEL setup, SOP, Memory, BiDir Streaming, Plugins)
  const otelSetupCode = generateOtelSetupCode(data);
  const agentSopCode = generateAgentSopCode(data);
  const memorySetupCode = generateMemorySetupCode(data);
  const bidirStreamingCode = generateBidirectionalStreamingCode(data);
  const goalLoopCode = generateGoalLoopCode(data);
  const humanInLoopCode = generateHumanInLoopCode(data);
  const mcpInstrCode = generateMcpInstrumentationCode(data);
  const preAgentSetup = [otelSetupCode, mcpInstrCode, agentControlSetup, customGuardrailHookCode, humanInLoopCode, agentSopCode, memorySetupCode, bidirStreamingCode, goalLoopCode].filter(Boolean).join('\n\n');
  const preAgentSetupStr = preAgentSetup ? `${preAgentSetup}\n` : '';

  // Add HITL hook to hooks list
  const hitlHookKwarg = getHumanInLoopHookKwarg(data);
  if (hitlHookKwarg) {
    hooks.push(hitlHookKwarg);
  }

  // Add community tools to the agent tools list
  const communityToolNames = getCommunityToolNames(data);
  if (communityToolNames.length > 0) {
    communityToolNames.forEach(t => {
      if (!allAgentToolCodes.includes(t)) {
        allAgentToolCodes.push(t);
      }
    });
    // Rebuild toolsCode with community tools included
  }
  const finalToolsCode = allAgentToolCodes.length > 0
    ? `,\n    tools=[${allAgentToolCodes.join(', ')}]`
    : '';

  // Bedrock guardrail kwargs inserted into model config comment
  // NOTE: Bedrock guardrail params are part of BedrockModel — handled in model-code-generator

  return `# ${label} Configuration
${modelConfig}
${structuredOutputDef}${preAgentSetupStr}
${agentVarName} = Agent(
    model=${agentVarName}_model,
    system_prompt="""${escapePythonTripleQuotedString(systemPromptValue)}"""${finalToolsCode},
    callback_handler=None${extraKwargsStr}
)`;
}
...
function findConnectedAgent(
  ...
  // Find agents/swarms/a2a/workflow connected to input nodes
  for (const inputNode of inputNodes) {
    ...
    for (const edge of connectedEdges) {
      ...
      if (targetNode && (targetNode.type === 'agent' || targetNode.type === 'orchestrator-agent' || targetNode.type === 'swarm' || targetNode.type === 'a2a-agent' || targetNode.type === 'workflow')) {
        return targetNode;
      }
    }
  }

  // Fallback: return first agent/orchestrator/swarm/a2a/workflow if no connections found
  const allExecutables = allNodes.filter(node => node.type === 'agent' || node.type === 'orchestrator-agent' || node.type === 'swarm' || node.type === 'a2a-agent' || node.type === 'workflow');
...
if __name__ == "__main__":
    ...
    asyncio.run(main(user_input_param, messages_param))`;
    } else if (executionAgent.type === 'a2a-agent') {
      // A2A Agent execution
      const a2aData = executionAgent.data || {};
      const a2aStreaming = a2aData.streaming || false;

      if (a2aStreaming) {
        mainCode += `
${baseIndent}# Execute A2A agent with streaming
...
${baseIndent}print()  # Final newline
${baseIndent}return ""

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Execute A2A Agent')
    parser.add_argument('--user-input', type=str, help='User input prompt')
    parser.add_argument('--messages', type=str, help='JSON string of conversation messages')
    ...
    asyncio.run(main(user_input_param, messages_param))`;
        ...
        mainCode += `
${baseIndent}# Execute A2A agent (sync)
${baseIndent}result = ${agentName}(user_input)
${baseIndent}print(str(result.message))
${baseIndent}return str(result.message)
...
if __name__ == "__main__":
    ...
    } else if (executionAgent.type === 'workflow') {
      // Workflow execution — already handled in generateWorkflowCode
      mainCode += `
${baseIndent}# Workflow execution is handled above via workflow tool
${baseIndent}# The workflow has already been created and started
${baseIndent}print("Workflow execution complete")
${baseIndent}return "Workflow execution complete"

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Execute Strands Workflow')
    parser.add_argument('--user-input', type=str, help='User input prompt')
    parser.add_argument('--messages', type=str, help='JSON string of conversation messages')
...
// ===== A2A Agent Code Generation =====

function generateA2AAgentCode(a2aNode: Node): string {
  const data = a2aNode.data || {};
  const {
    label = 'A2A Agent',
  ...
  const varName = sanitizePythonVariableName(label as string);

  let code = `# ${label} — Remote A2A Agent\n`;
  ...
  if (agentName) {
    params.push(`name="${agentName}"`);
  }
  if (description) {
    params.push(`description="${escapePythonTripleQuotedString(description as string)}"`);
  }
  if ((timeout as number) !== 300) {
  ...
  if (streaming) {
    code += `\n# To use streaming with this A2A agent:\n`;
    code += `# async for event in ${varName}.stream_async("your prompt here"):\n`;
    code += `#     if "data" in event:\n`;
    code += `#         print(event["data"], end="", flush=True)\n`;
  }
...
function generateWorkflowCode(workflowNode: Node): string {
  ...
  if ((tasks as WorkflowTask[]).length === 0) {
    return `# ${label} — No tasks defined\n# Add tasks in the Workflow node properties panel`;
  }

  let code = `# ${label} — DAG-based Workflow\n`;
  code += `${varName}_agent = Agent(tools=[workflow])\n\n`;

  // Generate tasks JSON
  code += `# Define workflow tasks\n`;
  code += `${varName}_tasks = [\n`;

  (tasks as WorkflowTask[]).forEach((task: WorkflowTask) => {
    code += `    {\n`;
    code += `        "task_id": "${task.taskId || 'unnamed_task'}",\n`;
    code += `        "description": "${(task.description || '').replace(/"/g, '\\"')}",\n`;
    if (task.systemPrompt) {
      code += `        "system_prompt": "${task.systemPrompt.replace(/"/g, '\\"')}",\n`;
    }
    if (task.dependencies && task.dependencies.length > 0) {
      code += `        "dependencies": [${task.dependencies.map(d => `"${d}"`).join(', ')}],\n`;
    }
    code += `        "priority": ${task.priority || 3}\n`;
    code += `    },\n`;
  });

  code += `]\n\n`;

  // Generate workflow creation and execution
  code += `# Create and execute workflow\n`;
  code += `${varName}_agent.tool.workflow(\n`;
  code += `    action="create",\n`;
  code += `    workflow_id="${workflowId}",\n`;
  code += `    tasks=${varName}_tasks\n`;
  code += `)\n\n`;
  code += `${varName}_agent.tool.workflow(action="start", workflow_id="${workflowId}")\n\n`;
  code += `# Check workflow status\n`;
  code += `${varName}_status = ${varName}_agent.tool.workflow(action="status", workflow_id="${workflowId}")\n`;
  code += `print(f"Workflow '${workflowId}' status: {${varName}_status}")\n`;
